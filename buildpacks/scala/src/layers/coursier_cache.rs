...
pub(crate) struct CoursierCacheLayer {
...
impl Layer for CoursierCacheLayer {
    ...
    fn create(
        ...
        log_info("Creating Coursier cache");
        LayerResultBuilder::new(GenericMetadata::default())
            .env(create_coursier_layer_env(
                layer_path,
                self.available_at_launch,
            ))
            .build()
    }
    ...
    fn existing_layer_strategy(
        ...
        log_info("Using existing Coursier cache");
        Ok(ExistingLayerStrategy::Keep)
    }
}

fn create_coursier_layer_env(layer_path: &Path, available_at_launch: Option<bool>) -> LayerEnv {
...
mod ivy_cache_layer_tests {
    ...
    #[test]
    fn create_ivy_layer_env_sets_ivy_flag_in_jvm_opts() {
        let layer_path = Path::new("./test_path");
        let layer_env = create_coursier_layer_env(layer_path, None);
