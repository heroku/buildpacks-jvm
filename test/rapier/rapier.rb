...
module Rapier
  ...
  class Runner
    ...
    def pack_build(app_dir, image_name: nil, buildpacks: [], build_env: {}, builder: nil, exception_on_failure: true)
      ...
      # image of the packaged buildpack.
      buildpacks = buildpacks.map do |buildpack|
        if File.directory?(buildpack) && File.file?("#{buildpack}/package.toml")
          buildpack_image_name = "packaged_bp_" + SecureRandom.hex(10)
          pack_command = "pack buildpack package #{buildpack_image_name} --config #{buildpack}/package.toml"

          _, pack_stderr, pack_status = Open3.capture3(pack_command)
          if pack_status != 0
            raise "While packaging meta-buildpack: pack exited with status code #{pack_status}, indicating an error and failed build!\nstderr: #{pack_stderr}"
          end

          "docker://#{buildpack_image_name}"
        else
          buildpack
        end
      end
